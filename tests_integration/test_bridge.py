"""What holds across kinds: the error boundaries, the issues, the export."""
...
from custom_components.config_bridge.kinds.http import HttpKind
...
async def test_a_kind_whose_ha_moved_under_it_does_not_stop_the_others(
...
async def test_a_kind_back_in_sync_clears_its_issue(
    hass: HomeAssistant, setup_bridge: SetupBridge
) -> None:
...
async def test_a_kind_taken_out_of_the_yaml_clears_its_issue(
    hass: HomeAssistant, setup_bridge: SetupBridge
) -> None:
    ir.async_create_issue(
        hass,
...
async def test_export_returns_every_kind_in_yaml_shape(
    hass: HomeAssistant, setup_bridge: SetupBridge
) -> None:
