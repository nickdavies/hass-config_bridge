...
State kept in the ledger: `owned`, the area ids the YAML listed when it last
matched, in either mode, for `owned` mode's deletes. Losing it leaves areas
the bridge managed before in place rather than deleting them.
"""
...
from .model import export_area, name_conflicts, normalize_name, render_area

OWNED: Final = "owned"
"""Ledger key: the area ids the bridge manages."""
...
class AreasKind(Kind):
    ...
    async def async_plan(self) -> Plan:
        ...
        collection = plan_collection(
            desired, live, mode=mode, owned=frozenset(self.state.get(OWNED, ()))
        )
    ...
    def _create(
        ...
        taken, or this wouldn't be a create) and renamed straight after: ids
        never change on rename.
        """
        name = fields["name"]
