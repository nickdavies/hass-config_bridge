"""Deciding what to do with Home Assistant's HTTP config store.

HTTP is the one kind the bridge can never apply on the boot it runs in: the
server binds in bootstrap stage 0, before any custom integration loads. HA's
own way of changing it is a two-slot store — `stable`, the last config known
to work, and `pending`, a trial for the next boot — plus a five-minute timer
that reverts an unpromoted trial and restarts. The bridge works with that
rather than around it: it stages the YAML as `pending` for the next restart,
and on the boot that runs it, promotes it before the timer fires. It never
restarts Home Assistant itself.
...
# The store's own vocabulary, as of storage version 2.2. The HTTP adapter
# checks Home Assistant still uses these exact values before trusting any
# decision made here.
