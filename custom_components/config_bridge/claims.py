"""Other integrations pinning their own entities and devices through the bridge.

An integration that creates entities with unique ids, or devices, gets
registry entries a person can edit in the UI: rename, move to another area,
hide. To keep those in git as well, it claims them at setup, in the shape of
the `entities` and `devices` object types' items:
...
claim says, with anything the claim leaves out put back to its default. One
the YAML lists too, or another integration claims, is reported rather than
pinned.

Claiming again replaces the integration's previous claim of that kind; one
it no longer claims is released at the next boot. Claims are read once Home
...
from .object_types.devices.model import ITEMS_SCHEMA as DEVICE_ITEMS
...
def claim_devices(
    hass: HomeAssistant,
    owner: str,
    devices: Mapping[str, Mapping[str, Mapping[str, Any] | None]],
) -> None:
    """Pin `devices`, by integration domain and identifier, for `owner`.

    The same shape as the `devices` object type's items. Raises `ValueError`
    if an item isn't valid; nothing is recorded then.
    """
    record_claim(hass.data, "devices", owner, devices, DEVICE_ITEMS)
