"""Planning a keyed collection: the items the YAML lists against the live ones.
...
alone. "Manages" has to survive a restart (the YAML stops listing an item
once it is removed from it), so the caller passes in the owned set it
...
class CollectionPlan:
    ...
    lists. Recorded in both modes, so a switch from exclusive to owned still
    knows what the bridge put there."""
