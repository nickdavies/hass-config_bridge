...
Every object type flattens its object to a mapping of field to value before
it gets here, so one differ serves an MQTT entry, the HTTP config and an area
alike.
...
class FieldChange:
    """One field that differs. `old` is what Home Assistant has."""
...
def diff_fields(
    ...
    *,
    ...
    YAML drops is reported (and removed when applied) rather than kept.
    """
